...
WebServer.prototype.configure = function(folder) {
  if (configuration('logging:level') !== 'none') this.app.use(logger());
  ...
  this.app.use(session({ store: new Store({ client: redis.connection }), rolling: true, cookie: { signed: false }}));
